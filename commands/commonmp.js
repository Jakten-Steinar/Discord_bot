...
module.exports = async function(msg, args){
    ...
    if(args.length > 1 && !(checkForDuplicates(args))){
        ...
        msg.channel.send('Finding common multi-player games for ' + names.join(' '))

        var userGames =  await steam.fetchMultiplayerGame(args);

        if(userGames.slice(0,3) == '@@@'){
            msg.channel.send(userGames.slice(3,userGames.length))
            msg.delete()
        }else{
            msg.channel.send('🎮 You have '+ userGames.length + ' common multi-player games 🎮')
